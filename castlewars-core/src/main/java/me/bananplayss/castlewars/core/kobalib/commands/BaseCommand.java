package me.bananplayss.castlewars.core.kobalib.commands;

import me.bananplayss.castlewars.core.kobalib.colors.ColorParser;
import me.bananplayss.castlewars.core.kobalib.commands.argumentMatchers.ArgumentMatcher;
...
public abstract class BaseCommand implements TabExecutor {
    ...
    protected final Component noPermMessage;
    protected final ArgumentMatcher argumentMatcher;

    public BaseCommand(String noPermissionMessage, ArgumentMatcher argumentMatcher) {
        this.noPermMessage = ColorParser.parse(noPermissionMessage);
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (args.length == 0) {
            ...
            if (helpSC != null && (sender.hasPermission(helpSC.getPermission()) || sender.isOp())) {
    ...
    public List<String> onTabComplete(CommandSender sender, Command command, String alias, String[] args) {
        ...
        if (args.length == 1) {
            List<String> subCommandsTC = subCommands.stream().filter(sc -> sender.hasPermission(sc.getPermission()) || sender.isOp()).map(SubCommand::getName).collect(Collectors.toList());
    ...
    public void registerMainCommand(JavaPlugin main, String cmdName) {
        ...
        cmd.permissionMessage(this.noPermMessage);
    }
